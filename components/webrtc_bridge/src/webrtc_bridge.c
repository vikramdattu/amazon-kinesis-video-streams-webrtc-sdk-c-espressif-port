...
static void webrtc_bridge_receive_callback(uint32_t msg_id, const uint8_t *data, size_t data_len)
{
...
void webrtc_bridge_start(void)
{
    ...
    esp_err_t ret = esp_hosted_register_custom_callback(WEBRTC_MSG_ID, webrtc_bridge_receive_callback);
    if (ret != ESP_OK) {
