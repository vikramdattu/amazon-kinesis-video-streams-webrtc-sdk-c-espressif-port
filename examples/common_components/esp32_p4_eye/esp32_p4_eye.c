...
#if CONFIG_BSP_AUDIO_SPEAKER_SUBBOARD
#define BSP_AUDIO_SUBBOARD_I2S_GPIO_CFG   \
    {                                     \
        .mclk = BSP_AUDIO_SUBBOARD_I2S_MCLK, \
        .bclk = BSP_AUDIO_SUBBOARD_I2S_BCLK, \
        .ws = BSP_AUDIO_SUBBOARD_I2S_LRCK, \
        .dout = BSP_AUDIO_SUBBOARD_I2S_DATA, \
        .din = GPIO_NUM_NC,               \
        .invert_flags = {                 \
            .mclk_inv = false,            \
            .bclk_inv = false,            \
...
#define BSP_AUDIO_SUBBOARD_I2S_MONO_CFG(_sample_rate)                                      \
    {                                                                                      \
        .clk_cfg = I2S_STD_CLK_DEFAULT_CONFIG(_sample_rate),                               \
        .slot_cfg = I2S_STD_PHILIP_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_16BIT,           \
                                                       I2S_SLOT_MODE_MONO),                \
        .gpio_cfg = BSP_AUDIO_SUBBOARD_I2S_GPIO_CFG,                                       \
    }
#endif
...
#if CONFIG_BSP_AUDIO_SPEAKER_SUBBOARD
static i2s_chan_handle_t i2s_audio_tx_chan;
static const audio_codec_data_if_t *i2s_audio_data_if = NULL;
#endif
...
#if CONFIG_BSP_AUDIO_SPEAKER_SUBBOARD
static esp_err_t bsp_audio_subboard_init(void)
{
    const gpio_config_t pa_io_config = {
        .pin_bit_mask = BIT64(BSP_AUDIO_SUBBOARD_PA_CTRL),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE
    };
    BSP_ERROR_CHECK_RETURN_ERR(gpio_config(&pa_io_config));
    BSP_ERROR_CHECK_RETURN_ERR(gpio_set_level(BSP_AUDIO_SUBBOARD_PA_CTRL, 0));
...
{
    if (i2s_audio_tx_chan) {
        return ESP_OK;
    }

    BSP_ERROR_CHECK_RETURN_ERR(bsp_audio_subboard_init());

    i2s_chan_config_t chan_cfg = I2S_CHANNEL_DEFAULT_CONFIG(BSP_I2S_NUM, I2S_ROLE_MASTER);
    chan_cfg.auto_clear = true;
    BSP_ERROR_CHECK_RETURN_ERR(i2s_new_channel(&chan_cfg, &i2s_audio_tx_chan, NULL));

    const i2s_std_config_t std_cfg_default = BSP_AUDIO_SUBBOARD_I2S_MONO_CFG(22050);
    const i2s_std_config_t *p_i2s_cfg = (i2s_config != NULL) ? i2s_config : &std_cfg_default;

    BSP_ERROR_CHECK_RETURN_ERR(i2s_channel_init_std_mode(i2s_audio_tx_chan, p_i2s_cfg));
    BSP_ERROR_CHECK_RETURN_ERR(i2s_channel_enable(i2s_audio_tx_chan));

    audio_codec_i2s_cfg_t i2s_cfg = {
        .port = BSP_I2S_NUM,
        .rx_handle = NULL,
        .tx_handle = i2s_audio_tx_chan,
    };
    i2s_audio_data_if = audio_codec_new_i2s_data(&i2s_cfg);
    if (i2s_audio_data_if == NULL) {
...
esp_codec_dev_handle_t bsp_audio_codec_speaker_init(void)
{
    ...
    es8311_codec_cfg_t es8311_cfg = {
        ...
        .pa_pin = BSP_AUDIO_SUBBOARD_PA_CTRL,
        .pa_reverted = false,
        .master_mode = false,
        .use_mclk = true,
        .digital_mic = false,
...
static const button_gpio_config_t bsp_button_gpio_config[BSP_BUTTON_NUM] = {
    {
        .gpio_num = BSP_BUTTON_NUM1,
        .active_level = 0,
    },
    {
        .gpio_num = BSP_BUTTON_NUM2,
        .active_level = 0,
    },
    {
        .gpio_num = BSP_BUTTON_NUM3,
        .active_level = 0,
    },
    {
        .gpio_num = BSP_BUTTON_ENCODER,
        .active_level = 0,
    },
};
...
/* Standard BSP API alias — media_stream and other consumers expect this name */
...
esp_codec_dev_handle_t bsp_audio_pdm_microphone_init(void)
{
    if (i2s_pdm_data_if == NULL) {
        ...
        if (bsp_pdm_audio_init(NULL) != ESP_OK) {
            return NULL;
        }
    }
...
{
    return bsp_audio_pdm_microphone_init();
}
...
esp_err_t bsp_iot_button_create(button_handle_t btn_array[], int *btn_cnt, int btn_array_size)
{
    ...
    for (int i = 0; i < BSP_BUTTON_NUM; i++) {
        ret = iot_button_new_gpio_device(&btn_cfg, &bsp_button_gpio_config[i], &btn_array[i]);
        if (ret != ESP_OK) {
            break;
        }
        if (btn_cnt) {
