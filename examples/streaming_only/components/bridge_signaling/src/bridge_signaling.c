...
static WEBRTC_STATUS bridgeInit(void *signaling_cfg, void **ppSignalingClient)
{
    ...
    /* Initialize ICE client internally so the application doesn't need to
     * call ice_bridge_client_init() directly. bridge_cmd_init() is left
     * to the example since examples register their own commands. */
#if CONFIG_ESP_WEBRTC_BRIDGE_HOSTED
    ice_bridge_client_init();
#endif
...
/**
 * @brief Background task for refreshing ICE servers via bridge (async model)
 *
 * Triggers async request to signaling. Responses arrive via bridge_cmd/signaling;
 * ice_bridge_client adds them to cache and invokes bridge_on_ice_servers_updated,
 * which notifies the app via on_ice_servers_updated.
 */
static void bridge_refresh_ice_task(void *arg)
{
    ...
    (void)arg;  /* Callback receives pClientData as user_data */
    ESP_LOGI(TAG, "Background bridge ICE refresh task: requesting async");

    ice_bridge_client_clear_cache();
    ice_bridge_client_request_ice_servers_async();
    /* Servers will arrive asynchronously; bridge_on_ice_servers_updated notifies app */
}

/**
 * @brief Internal callback when ice_bridge_client receives new ICE servers
 *
 * Copies cache into client data and invokes app's on_ice_servers_updated.
 */
static void bridge_on_ice_servers_updated(uint32_t count, void *user_data)
{
    BridgeSignalingClientData *pClientData = (BridgeSignalingClientData *)user_data;
    if (pClientData == NULL) {
        return;
    }

    ice_bridge_client_get_cached_servers(pClientData->ice_servers_config.ice_servers,
                                         &pClientData->ice_servers_config.ice_server_count);
    pClientData->ice_servers_received = true;

    if (pClientData->ice_callback_ctx.on_ice_servers_updated != NULL && count > 0) {
        ESP_LOGI(TAG, "Progressive ICE: notifying app about %" PRIu32 " ICE servers", count);
        pClientData->ice_callback_ctx.on_ice_servers_updated(pClientData->ice_callback_ctx.customData, count);
    }
}
                                                ...
                                                WEBRTC_STATUS (*on_ice_servers_updated)(uint64_t, uint32_t))
{
    ...
    ice_bridge_client_set_servers_updated_callback(bridge_on_ice_servers_updated, pClientData);

    ESP_LOGD(TAG, "Bridge ICE update callback set");
    return WEBRTC_STATUS_SUCCESS;
}
